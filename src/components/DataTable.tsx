...
export default function DataTable({ onRowClick, searchQuery, className }: DataTableProps) {
  ...
  const handleFiltersChange = useCallback((newFilters: Record<string, string | number | boolean | undefined>) => {
        ...
        <div className="px-4 py-3 border-b bg-gray-50 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div className="text-sm text-gray-700">
              <span className="font-medium">{data.total}</span> entries found
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {data.data.map((entry) => (
                <tr
                  key={entry.id}
                  className={`hover:bg-gray-50 transition-colors ${selection.selectedIds.has(entry.id) ? 'bg-blue-50' : ''}`}
                >
                  ...
                  >
                    ...
                    {entry.isMwe && (
                      <span className="inline-block mt-1 px-2 py-0.5 text-xs bg-yellow-100 text-yellow-800 rounded-md">
                        MWE
                      </span>
                    )}
                    {entry.transitive && (
                      <span className="inline-block mt-1 ml-1 px-2 py-0.5 text-xs bg-purple-100 text-purple-800 rounded-md">
                        Transitive
                      </span>
                    )}
                  </td>
                  ...
                  <td 
                    ...
                    onClick={() => onRowClick?.(entry)}
                  >
                    <span className="text-xs text-gray-500 font-mono">{entry.lexfile}</span>
                  </td>
                  <td 
                    className={`px-4 py-4 whitespace-nowrap ${onRowClick ? 'cursor-pointer' : ''}`}
                    onClick={() => onRowClick?.(entry)}
        ...
        <div className="px-4 py-3 border-t bg-gray-50 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-4 py-2 text-sm font-medium border border-gray-300 rounded-md hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed bg-white text-gray-700 transition-colors"
            >
              ...
              {Array.from({ length: Math.min(5, data.totalPages) }, (_, i) => {
                ...
                return (
                  <button
                    ...
                    disabled={loading}
                    className={`px-4 py-2 text-sm font-semibold border rounded-md min-w-[40px] transition-colors ${
                      pageNum === data.page
