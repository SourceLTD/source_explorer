...
function PlanKindRenderer({ plan }: { plan: IssueChangePlanSummary }) {
  ...
  switch (plan.plan_kind) {
    case 'split_frame': {
      // Phase 6 metadata (from `normaliseSplitFrame` in the runner):
      ...
      // Result frames have NO real id yet (they're created on
      // commit; the runner threads `placeholder_id` through so we
      // can surface "new frame #-1" on the card without coupling
      // to commit-time id resolution). The renderer surfaces:
      //   - per-result label + LLM-suggested definition
      //   - per-result sense count and parent count (the cleanup
      //     fan-out the LLM picked)
      //   - the role_labels chip strip already supported
      //   - a "Structural cleanup" footer explaining the
      //     stale_role_mappings deletes (regenerated by the next
      //     FRAME_INHERITANCE_MISSING_ROLE_MAPPINGS sweep).
      ...
      const totalParentEdges = (results ?? []).reduce(
        (sum, r) =>
          sum + (Array.isArray(r.inherits_from) ? r.inherits_from.length : 0),
        0,
      );
      return (
              ...
              results && results.length > 0 ? (
                results.map((r, i) => {
                  ...
                  const senseIds = Array.isArray(r.sense_ids)
                    ...
                    : [];
                  const inheritsFrom = Array.isArray(r.inherits_from)
                    ? (r.inherits_from as unknown[]).filter(
                        (v): v is string => typeof v === 'string',
                      )
                    : [];
                  return (
                            ...
                            {inheritsFrom.length > 0 && (
                              <div className="space-y-0.5">
                                ...
                                <div className="flex flex-wrap gap-1">
                                  {inheritsFrom.map((parentId) => (
                                    <FrameChip
                                      key={parentId}
                                      frameId={parentId}
                                      label={`#${parentId}`}
                                      className="font-mono text-[10px] px-1.5 py-0.5 rounded bg-white border border-blue-200 text-blue-800"
                                    />
                                  ))}
                                </div>
                              </div>
          ...
          {(totalSenseRepoints > 0 ||
            ...
            <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
              ...
              <ul className="list-disc list-inside space-y-0.5">
                ...
                {totalParentEdges > 0 && (
                  <li>
                    {totalParentEdges} <code className="font-mono">parent_of</code>{' '}
                    edge
                    {totalParentEdges === 1 ? ' will be' : 's will be'} created
                    so each new frame inherits from the listed parents.
                  </li>
                )}
                {staleMappingIds.length > 0 && (
                  <li>
